    ...
    public void fillConsul(Path file, String profile, String key, String value) {
        if (file != null) {
            try (InputStream input = new FileInputStream(file.toFile())) {
                ...
                PropertiesProvider propertiesProvider = consulConfiguration.propertiesProviderSelector()
                        .getProvider(file.getFileName().toString());
                properties.putAll(propertiesProvider.getProperties(input));

                sendProperty(profile, properties);
            } catch (IOException e) {
                LOGGER.error("Unable to load properties from file {} due to {}", file, e.getMessage());
            }
        } else {
            Properties properties = new Properties();
